...
/**
 * /health отвечает 200, пока процесс обрабатывает запросы. /ready отвечает
 * 200 только когда база отвечает на запрос. Разделение нужно деплою: по
 * /health процесс перезапускают, по /ready убирают из балансировки.
 *
 ...
 *
 * Замечание к реализации: у pg.Pool нужно слушать событие 'error'. Без
 * слушателя ошибка простаивающего соединения — необработанная ошибка Node,
 * то есть завершение процесса вместо ответа 503.
 */
